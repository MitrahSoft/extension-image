...
public final class Image extends TagImpl {
	...
	public void setAction(String strAction) throws PageException {
		...
		else throw eng.getExceptionUtil().createApplicationException("invalid action ["+this.strAction+"], " +
	...
	public void setFormat(String format) throws PageException {
		...
		else throw eng.getExceptionUtil().createApplicationException("invalid format ["+format+"], " +
	...
	private void doActionCaptcha() throws PageException, IOException {
		...
		source=new org.lucee.extension.image.Image(c.generate(text, eng.getCastUtil().toIntValue(width),eng.getCastUtil().toIntValue(height), fonts, true, 
	...
	private void writeLink(String path) throws IOException, PageException {
		...
		if(base64) {
			...
			pageContext.write("<img src=\"data:"+ImageUtil.getMimeTypeFromFormat(format)+";base64,"+b64+"\" width=\""+this.width+"\" height=\""+this.height+"\""+add+" />");
			return;
		}
		pageContext.write("<img src=\""+path+"\" width=\""+this.width+"\" height=\""+this.height+"\""+add+" />");
	...
	private int toDimension(String label, String dimension, Info info) throws PageException {
		...
		if(dimension.endsWith("%")) {
			float pro=eng.getCastUtil().toIntValue(
